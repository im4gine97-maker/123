...
def fetch_governance_criticism(tk, cd, ceo_name):
    ...
    db = {
        ...
        "XOM": "대런 우즈 (Darren Woods): 효율적인 자본 배분과 주주환원에 철저한 경영진이나, 화석 연료의 환경 규제 및 유가 변동성이 치명적 리스크입니다. (이헌 확인이 필요한 부분입니다)",
...
# 🚀 [연구개발비(R&D) 4개년 추이 및 급증 알림 기능 추가 보완]
        ...
        if inc is not None and not inc.empty and 'Research And Development' in inc.index:
            ...
            if not rnd_series.empty:
                ...
                if curr_rnd > 0:
                    ...
                    # ⚠️ 최근 1년 전방 대비 30% 이상 갑자기 늘었는지 체크 (급증 판정)
                    sudden_alert = ""
                    if len(rnd_vals) >= 2:
                        prev_rnd = safe_float(rnd_vals[-2])
                        if prev_rnd > 0:
                            inc_pct = ((curr_rnd - prev_rnd) / prev_rnd) * 100
                            if inc_pct >= 30:
                                sudden_alert = f" <span class='highlight'>[⚠️ 최근 {inc_pct:.1f}% 급증!]</span>"
...
with tab5:
    ...
    phil_li3 = t("**경영진의 정직성 (Integrity of Management):** 재무적 성과만큼이나 중요한 것이 경영진의 도덕성입니다. 비즈니스 모델이 훌륭해도 경영진의 정직성에 의구심이 든다면 미련 없이 동업을 끝내야 합니다. 신뢰할 수 없는 사람과는 좋은 거래 파트너가 시될 수 없습니다.", "**Integrity of Management:** Management's morality is just as important as financial performance. Even if the business is great, if you doubt their integrity, you must walk away. You cannot make a good deal with a bad person.")
