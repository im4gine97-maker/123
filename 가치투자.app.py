    ...
    if st.session_state.search_tk:
        ...
        with st.spinner(t("데이터 스캔 중...", "Scanning Data...")):
            ...
            if p:
                ...
                # 4. 질적 분석 (뉴스 요약 및 리스크 비판 섹션 통합 심화)
                ...
                try:
                    ...
                    if news_items:
                        for item in news_items[:3]:
                            ...
                            st.markdown(f"- [{tr_text(n_title)}]({n_link}) *(Source: {n_pub})*")
                    else:
                        st.caption(t("최근 뉴스가 존재하지 않습니다.", "No recent news found."))
                except:
                    st.caption(t("뉴스 피드를 연동하지 못했습니다.", "Failed to load real-time news feed."))

                # 💡 경영진 비판 및 공공 리스크 스크리닝 항목
                st.write(t("**경영진 및 지배구조 비판 점검 패널**", "**Management & Governance Criticism Panel**"))
                st.warning(t("⚠️ 공공 기록 및 위키 데이터 연동 경고 (이건 확인이 필요한 부분입니다):\n\n"
                             "- **지배구조/독점 리스크:** 규제 당국의 반독점 규제 소송 및 벌금 리스크 모니터링 필요.\n"
                             "- **자본배분 비판:** 경영진의 과도한 스톡옵션 발행(주주가치 희석) 및 부적절한 M&A 이력이 있는지 확인 요망.\n"
                             "- **이해관계자 평판:** 임직원 처우, 노사 갈등, 공급망 ESG 리스크에 대한 교차 검증이 요구됨.",
                             "⚠️ Public Records & Wiki Check Warning (This part needs verification):\n\n"
                             "- **Governance/Antitrust:** Monitor regulatory antitrust lawsuits and potential fine risks.\n"
                             "- **Capital Allocation:** Check for historical shareholder dilution via stock options or value-destructive M&As.\n"
                             "- **Stakeholder Reputation:** Labor disputes, internal workforce treatment, and supply chain ESG screening required."))
...
with tab5:
    ...
    terms = [
        ...
        ("Fwd PER (선행 주가수익비율)", 
         ...
         t("주식은 미래의 가치를 반영하므로 단순 PER보단 Fwd PER이 더 중요합니다.", "Since stocks reflect future value, Fwd PE is more important than trailing PE.")),
